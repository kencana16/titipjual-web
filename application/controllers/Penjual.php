...
class Penjual extends CI_Controller {
	...
	public function edit_penjual($id = null){
		if(!isset($id)) show_404();

		$validation = $this->form_validation->set_rules($this->penjual_models->rules());
		if($validation->run()){
			if($this->penjual_models->edit($id)) $this->session->set_flashdata('success', 'Berhasil Diupdate');
            redirect(current_url());
		}
	...
	public function hapus_penjual($id = null){
		if(!isset($id)) show_404();
